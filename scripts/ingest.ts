...
const CHUNKS: Chunk[] = [
  ...
  {
    id: 'jadwal-baptisan',
    content: 'Jadwal Baptisan Air GBI Baranangsiang yang tersedia: 14 Maret 2026, 9 Mei 2026, 18 Juli 2026, 12 September 2026, 7 November 2026. Pendaftaran dapat dilakukan melalui formulir di website. Pendaftaran dan kegiatan Baptisan Air tidak dipungut biaya (gratis).',
    metadata: { category: 'baptisan', source: 'admin-dates', type: 'jadwal' },
  },
  {
    id: 'jadwal-ibadah',
...
function loadChunks(): Chunk[] {
  // If a JSON file is passed as argument, load from it instead of hardcoded chunks
  ...
  if (jsonPath) {
    ...
    const docs: ExportedDoc[] = JSON.parse(readFileSync(resolved, 'utf-8'));
    return docs.map(d => ({
      id: d.id,
      content: d.content,
...
async function main() {
  ...
  const source = process.argv[2] ? `from ${process.argv[2]}` : 'from hardcoded chunks';

  console.log(`\n🏛️  GBI BEC Document Ingestion`);
  console.log(`${'='.repeat(40)}`);
  console.log(`📄 Chunks to ingest: ${chunks.length} (${source})`);
