...
import { Loader2, RefreshCw, Trash2, MessageCircleQuestion, Search, User, Bot, MessagesSquare } from 'lucide-react';
...
interface ChatMiss {
  ...
  /** Last 4 messages before this question — empty for entries logged before this existed. */
                    ...
                    >
                      <TableCell className="text-sm font-medium max-w-xs">
                        <div className="flex items-start gap-1.5">
                          {miss.context?.length > 0 && (
                            <MessagesSquare
                              className="w-3.5 h-3.5 text-muted-foreground shrink-0 mt-0.5"
                              aria-label="Ada konteks percakapan sebelumnya"
                            >
                              <title>Ada {miss.context.length} pesan konteks sebelum pertanyaan ini</title>
                            </MessagesSquare>
                          )}
          ...
          <DialogContent className="sm:max-w-2xl">
            ...
            {selected && (
              <DialogBody className="space-y-4">
                {/* Prior conversation — what this question was actually a follow-up to.
                    A miss like "jadi bagaimana yaa" is meaningless without this. */}
                    ...
                    <div className="space-y-1.5">
                      {selected.context.map((m, i) => (
                        <div key={i} className="flex gap-2 text-xs">
                          <span className="font-medium text-muted-foreground shrink-0 w-14">
                            {m.role === 'user' ? 'Pengguna:' : 'Asisten:'}
                          </span>
                          <span className="text-foreground/80 leading-relaxed whitespace-pre-wrap">{m.content}</span>
                        </div>
                      ))}
                    </div>
                  </div>
